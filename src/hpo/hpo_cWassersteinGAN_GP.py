"""
NAME
    hpo_cWassersteinGAN_GP

DESCRIPTION
    This module provides an optuna-based hyperparameter optimization method for the cWassersteinGAN_GP architecture.
    It can optimize the following models within the cWassersteinGAN_GP architecture:
        Generators: MLP, STB_MLP, Transformer
        Discriminators: MLP, MHDB_MLP, Transformer
"""

import optuna
from training.train_cWassersteinGAN_GP import train_cWassersteinGAN_GP
import globals.constants as const


def objective_cWassersteinGAN_GP(trial):
    """ Objective function for tuning the cWassersteinGAN_GP in the optuna-framework. """
    ...
    # lambda_gp = trial.suggest_int('lambda_gp', 10, 15)
    lambda_gp = 10
    ...
    try:
        metric = train_cWassersteinGAN_GP(config)
...
def hpo_cWassersteinGAN_GP(n_trials=10):
    """
    Starts HPO for cWassersteinGAN_GP architecture.
    ...
    # Optimize
    study.optimize(objective_cWassersteinGAN_GP, n_trials=n_trials, gc_after_trial=True)
